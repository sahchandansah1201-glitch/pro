...
function ProductionClinicalWorkspacePanel({
  ...
  const loadReportDetails = useCallback(async () => {
    if (kind !== "report" || reportDetailsLoading) return;
    setStatus("");
    setReportDetailsLoading(true);
    const args = { apiBaseUrl, apiToken, visitId };
    const [auditResult, viewerQaQueueResult, longitudinalDatasetValidationResult] = await Promise.all([
      getSelfHostedPatientPhotoProtocolReleaseAudit(args),
      getSelfHostedVisitLesionComparisonViewerQaReviewQueue({
        ...args,
        status: "actionable",
        limit: 20,
      }),
      getSelfHostedVisitLongitudinalDatasetValidation(args),
    ]);
    setReportDetailsLoading(false);
    ...
    if (!auditResult.ok || !viewerQaQueueResult.ok || !longitudinalDatasetValidationResult.ok) {
      setStatus("Часть подробного контроля временно недоступна. Основной отчёт открыт.");
      return;
    }
    setStatus("Подробный контроль загружен.");
  }, [apiBaseUrl, apiToken, kind, reportDetailsLoading, visitId]);
              ...
              <div className="rounded-sm border border-border bg-surface px-3 py-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-[13px] font-semibold">Подробный контроль отчёта</h3>
                    <p className="mt-1 text-[12px] text-muted-foreground">
                      Основной отчёт открыт. Журнал выдачи фото и проверка истории загружаются по запросу врача.
                    </p>
                  </div>
                  <Button
                    type="button"
                  ...
                  >
                    {reportDetailsLoading ? "Загружаем…" : "Открыть подробный контроль"}
                  </Button>
                </div>
              </div>
            )}

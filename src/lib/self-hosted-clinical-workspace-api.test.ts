...
describe("self-hosted-clinical-workspace-api", () => {
  ...
  it("reviews production reviewer governance through metadata-only Stage 5H contract", async () => {
    const fetchMock = vi.fn(async () =>
      new Response(
        JSON.stringify({
          item: {
            id: "production-reviewer-governance-1",
            clinicId: "clinic-1",
            patientId: "patient-1",
            visitId: "visit-1",
            status: "in_review",
            ...
            protectedFieldsExposed: true,
            clinicalOutputGenerated: true,
            rawProductionReviewerGovernanceLog: "unsafe",
            productionReviewerGovernancePayload: { unsafe: true },
            reviewerName: "Unsafe Name",
            reviewerEmail: "unsafe@example.com",
            pairKey: "secret-pair",
            imageIds: ["i-011", "i-012"],
    ...
    expect(result.value?.clinicalOutputGenerated).toBe(false);
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:3001/api/v1/visits/visit-1/longitudinal-timeline-rollout/production-reviewer-governance",
      expect.objectContaining({
        method: "PATCH",
        body: JSON.stringify({
          productionReviewerGovernanceStatus: "ready_for_production_reviewer_governance",

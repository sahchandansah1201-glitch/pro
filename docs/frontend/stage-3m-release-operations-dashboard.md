...
`/sys/release-status` now includes a browser-side `CI status page` and
`Release readiness dashboard` above the raw release-status preview.

The readiness summary is built by `buildReleaseReadinessSummary` in
`src/lib/release-status-ui.ts`. It combines:

- working-tree cleanliness;
- `deno.lock` guard status;
- release-status artifact presence;
- tracked workflow conclusions, including `preflight-all`, `typecheck`, and
  `typecheck-blob-verification`;
- the browser write-gate drill result from
  `buildReleaseStatusWriteGateSummary`.

The same helper produces the operator notification text. A ready state says the
release report link may be published after human review. A blocked state emits
`Gate failed: reports stay unwritten` and lists the failing checks. The page
renders that blocked state as `Gate failure notification release status`, so
Playwright can assert that failed gates are visible to operators before any
report-write step is trusted.

The publishable report URL is generated by `buildReleaseReportArtifactUrl`. It
points to the `release-status` GitHub Actions run artifacts (`#artifacts`) when
a workflow URL is available, otherwise it falls back to the safe commit URL.
The page exposes the link as `Открыть опубликованный release readiness report`
and provides a copy action for PR comments or release notes.
...
- RBAC, export, release comparison, history-import, history-preview,
  ...
  ci-status-page, release-readiness-dashboard, report-link-publish,
  gate-failure-notification, and release-status-sync-checker changes must also keep

...
Stage 25A-25Z is the current product-facing batch. It extends Stage 24A-24Z local SOP policy exception closure with local SOP policy audit rollup metadata on follow-up tasks.

## Immediate actions after Stage 25A-25Z PR merge
...
Stage 26A-26Z is a hypothesis until repository files define it. A likely next product cycle is follow-up policy audit export or clinic governance review readiness, but that is not confirmed until repository files define it.

...
1. **Readiness/decision/observation/closure/archive packages != live go-live approval**
   - Evidence: Stage 6E/6F/6G/6H/6I/6J/6K/6L/6M report outputs include:
...
2. **Operational dependence on external operator-run steps**
   - Evidence: Stage 6B-6M manifests include deploy/smoke/backup/rollback,
     live evidence, handoff, decision-record, observation, closure, archive,
     receipt, reconciliation, reconciliation receipt, and final closure gates.
...
1. **Next stage ambiguity**
   - Hypothesis: Stage 6N is next.
   - Basis: Stage 6A-6M exist in the current branch; Stage 6N files are not
     confirmed.
   - Uncertainty: no explicit roadmap file in current scan naming Stage 6N
     scope.
...
3. Keep live evidence, live logs, live metrics, final go-live approval, and
   final observation/closure/archive/reconciliation/reconciliation receipt/final closure outcomes outside git,
   referenced only by deterministic redacted receipt/handoff/decision-record/
   observation/closure/archive/reconciliation fields.

...
import {
  ...
  buildReviewVisitLongitudinalTimelineRolloutProductionReviewerEvidenceSql,
...
test("Batch BZ Stage 5H repository builds metadata-only exception governance SQL", () => {
  const sql = buildReviewVisitLongitudinalTimelineRolloutExceptionGovernanceSql({
    visitId: VISIT_ID,
    patientId: PATIENT_ID,
    clinicId: CLINIC_ID,
    doctorUserId: USER_ID,
  ...
  assert.match(sql, /insert into visit_longitudinal_timeline_rollout_exception_governance_reviews/);
  assert.match(sql, /exception_governance_status/);
  assert.match(sql, /ready_for_exception_governance/);
  assert.match(sql, /timelineRolloutExceptionGovernanceBoundary/);
  assert.match(sql, /exceptionGovernanceBoundary/);
  assert.match(sql, /recurrenceReviewBoundary/);
  assert.match(sql, /rollbackReadinessBoundary/);
  assert.match(sql, /patientDeliveryAllowed/);
  assert.match(sql, /medicalMeasurementAllowed/);
  assert.match(sql, /protectedFieldsExposed/);
  assert.match(sql, /clinicalOutputGenerated/);
...
test("Batch CC Stage 5H repository builds metadata-only protected reviewer validation SQL", () => {
  const sql = buildReviewVisitLongitudinalTimelineRolloutProtectedReviewerValidationSql({
    visitId: VISIT_ID,
    patientId: PATIENT_ID,
    clinicId: CLINIC_ID,
    doctorUserId: USER_ID,
  ...
  assert.match(sql, /insert into visit_longitudinal_timeline_rollout_protected_reviewer_validation_reviews/);
  assert.match(sql, /protected_reviewer_validation_status/);
  assert.match(sql, /ready_for_protected_reviewer_validation/);
  assert.match(sql, /timelineRolloutProtectedReviewerValidationBoundary/);
  assert.match(sql, /protectedReviewerValidationBoundary/);
  assert.match(sql, /protectedAssetReviewBoundary/);
  assert.match(sql, /patientDeliveryAllowed/);
  assert.match(sql, /medicalMeasurementAllowed/);
  assert.match(sql, /protectedFieldsExposed/);
  assert.match(sql, /clinicalOutputGenerated/);
...
test("Batch CF Stage 5H repository builds metadata-only production dataset evidence SQL", () => {
  const sql = buildReviewVisitLongitudinalTimelineRolloutProductionDatasetEvidenceSql({
    visitId: VISIT_ID,
    patientId: PATIENT_ID,
    clinicId: CLINIC_ID,
    doctorUserId: USER_ID,
  ...
  assert.match(sql, /insert into visit_longitudinal_timeline_rollout_production_dataset_evidence_reviews/);
  assert.match(sql, /production_dataset_evidence_status/);
  assert.match(sql, /ready_for_production_dataset_evidence/);
  assert.match(sql, /timelineRolloutProductionDatasetEvidenceBoundary/);
  assert.match(sql, /productionDatasetEvidenceBoundary/);
  assert.match(sql, /clinicOperationEvidenceBoundary/);
  assert.match(sql, /longitudinalEvidenceBoundary/);
  assert.match(sql, /patientDeliveryAllowed/);
  assert.match(sql, /medicalMeasurementAllowed/);
  assert.match(sql, /protectedFieldsExposed/);
  assert.match(sql, /clinicalOutputGenerated/);
...
test("Batch CH Stage 5H repository builds metadata-only production reviewer evidence SQL", () => {
  const sql = buildReviewVisitLongitudinalTimelineRolloutProductionReviewerEvidenceSql({
    visitId: VISIT_ID,
    patientId: PATIENT_ID,
    clinicId: CLINIC_ID,
    doctorUserId: USER_ID,
    productionReviewerEvidence: {
      ...
      ownerSignoffStatus: "ready",
      productionReviewWindowCount: 8,
      evidenceRecordedProductionReviewerCount: 6,
      sampledProductionReviewerCount: 5,
      adjudicatedProductionReviewerCount: 4,
      followupEvidenceProductionCount: 4,
      exceptionEvidenceProductionCount: 2,
      rollbackEvidenceProductionCount: 1,
  ...
  assert.match(sql, /visit_longitudinal_timeline_rollout_production_reviewer_evidence_reviews/);
  assert.match(sql, /ready_for_production_reviewer_evidence/);
  assert.match(sql, /production_reviewer_governance_status/);
  assert.match(sql, /production_reviewer_assignment_status/);
  assert.match(sql, /assigned_production_reviewer_count/);
  assert.match(sql, /patientDeliveryAllowed/);
  assert.match(sql, /medicalMeasurementAllowed/);
  assert.match(sql, /protectedFieldsExposed/);
  assert.match(sql, /clinicalOutputGenerated/);
  assert.doesNotMatch(
    sql,
    /"pairKey"\s*:|"imageIds"\s*:|objectBucket|objectKey|storagePath|storageObjectPath|signedUrl|rawProductionReviewerEvidenceLog|productionReviewerEvidencePayload|productionReviewerOpsPayload|reviewerName|reviewerEmail|validatorName|validatorEmail|doctorVersionText|patientSafeText|dynamicConclusion|diagnosis|riskScore/i,
  );
});

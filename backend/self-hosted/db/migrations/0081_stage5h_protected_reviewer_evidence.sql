-- Stage 5H · Batch CE
-- Metadata-only long-running evidence gate on real protected reviewer
-- operations after protected reviewer governance. Stores aggregate evidence
...
create table if not exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews (
  ...
  protected_reviewer_evidence_reasons jsonb not null default '[]'::jsonb,
  ...
  reviewer_monitoring_evidence_status text not null default 'missing',
  reviewer_exception_evidence_status text not null default 'missing',
  reviewer_adjudication_evidence_status text not null default 'missing',
  reviewer_followup_evidence_status text not null default 'missing',
  reviewer_rollback_evidence_status text not null default 'missing',
  reviewer_archive_evidence_status text not null default 'missing',
  ...
  sampled_protected_review_count integer not null default 0,
  adjudicated_protected_evidence_count integer not null default 0,
  followup_closed_protected_count integer not null default 0,
  rollback_drill_protected_count integer not null default 0,
  archived_protected_review_count integer not null default 0,
  unresolved_protected_evidence_count integer not null default 0,
...
alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_status_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_status_check
    check (
...
alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_validation_status_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_validation_status_check
    check (
...
alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_checklist_status_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_checklist_status_check
    check (
      reviewer_monitoring_evidence_status in ('missing', 'needs_review', 'ready')
      and reviewer_exception_evidence_status in ('missing', 'needs_review', 'ready')
      and reviewer_adjudication_evidence_status in ('missing', 'needs_review', 'ready')
      and reviewer_followup_evidence_status in ('missing', 'needs_review', 'ready')
      and reviewer_rollback_evidence_status in ('missing', 'needs_review', 'ready')
      and reviewer_archive_evidence_status in ('missing', 'needs_review', 'ready')
      and owner_signoff_status in ('missing', 'needs_review', 'ready')
    );

alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_counts_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_counts_check
    check (
      protected_review_window_count >= 0
      and monitored_protected_review_count >= 0
      and sampled_protected_review_count >= 0
      and adjudicated_protected_evidence_count >= 0
      and followup_closed_protected_count >= 0
      and rollback_drill_protected_count >= 0
      and archived_protected_review_count >= 0
      and unresolved_protected_evidence_count >= 0
      and blocker_count >= 0
      and lesion_count >= 0
      ...
      and sampled_protected_review_count <= monitored_protected_review_count
      and adjudicated_protected_evidence_count <= monitored_protected_review_count
      and followup_closed_protected_count <= adjudicated_protected_evidence_count
      and rollback_drill_protected_count <= monitored_protected_review_count
      and archived_protected_review_count <= monitored_protected_review_count
      and unresolved_protected_evidence_count <= monitored_protected_review_count
    );

alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reasons_array_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reasons_array_check
    check (jsonb_typeof(protected_reviewer_evidence_reasons) = 'array');

alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_boundary_check,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_boundary_check
    check (
...
alter table visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_protected_reviewer_evidence_metadata_no_protected_keys,
  add constraint visit_longitudinal_timeline_rollout_protected_reviewer_evidence_metadata_no_protected_keys check (
    not (
      metadata_json ?| array[
        'pairKey',
        'imageIds',
        ...
        'signedUrl',
        'sharedLink',
        'protectedReviewerEvidenceUrl',
        'reviewerMonitoringEvidenceUrl',
        'reviewerExceptionEvidenceUrl',
        'reviewerAdjudicationEvidenceUrl',
        'reviewerFollowupEvidenceUrl',
        'reviewerRollbackEvidenceUrl',
        'reviewerArchiveEvidenceUrl',
        ...
        'reviewerAdjudicationEvidencePayload',
        'reviewerAdjudicationEvidenceDetails',
        'reviewerFollowupEvidencePayload',
...
create index if not exists idx_visit_longitudinal_timeline_rollout_protected_reviewer_evidence_visit
  on visit_longitudinal_timeline_rollout_protected_reviewer_evidence_reviews (visit_id, updated_at desc);

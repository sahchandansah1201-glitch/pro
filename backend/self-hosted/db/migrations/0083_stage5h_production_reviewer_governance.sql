-- Stage 5H · Batch CG
-- Metadata-only production reviewer governance over time on production assets
...
create table if not exists visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews (
  ...
  production_reviewer_governance_reasons jsonb not null default '[]'::jsonb,
  ...
  unresolved_production_reviewer_governance_count integer not null default 0,
...
alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_status_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_status_check
    check (
      production_reviewer_governance_status in (
        'not_started',
        'in_review',
...
alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_previous_status_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_previous_status_check
    check (
...
alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_checklist_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_checklist_check
    check (
...
alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_counts_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_counts_check
    check (
      ...
      and unresolved_production_reviewer_governance_count >= 0
      and blocker_count >= 0
      and lesion_count >= 0
      ...
      and unresolved_production_reviewer_governance_count <= assigned_production_reviewer_count
    );

alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_reasons_array_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_reasons_array_check
    check (jsonb_typeof(production_reviewer_governance_reasons) = 'array');

alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_boundary_check,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_boundary_check
    check (
...
alter table visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews
  drop constraint if exists visit_longitudinal_timeline_rollout_production_reviewer_governance_metadata_no_protected_keys,
  add constraint visit_longitudinal_timeline_rollout_production_reviewer_governance_metadata_no_protected_keys check (
    not (
      metadata_json ?| array[
        'pairKey',
        'imageIds',
...
create index if not exists visit_longitudinal_timeline_rollout_production_reviewer_governance_clinic_status_idx
  on visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews (
    clinic_id,
    production_reviewer_governance_status,
    updated_at desc
  );

create index if not exists visit_longitudinal_timeline_rollout_production_reviewer_governance_visit_idx
  on visit_longitudinal_timeline_rollout_production_reviewer_governance_reviews (
    visit_id,
    updated_at desc
  );

 ...
 *   Body map → Снимки → Оценка → Заключение → Отчёт.
 *
 * Production-код не трогаем. Используем только публичные ARIA-локаторы и
 * допустимый demo-role в localStorage (как в других admin pw-тестах).
...
test.describe("Visit Workspace mobile@390 — без горизонтального скролла", () => {
  ...
  test("Report → переключение по всем вкладкам и обратно", async ({ page }) => {
    ...
    // 3. Демо-кнопки отчёта существуют и задизейблены.
    const printBtn = page
      .getByRole("button", { name: /Печать \/ PDF \(демо\)/ })
      .first();
    ...
    for (const tab of TABS) {
      ...
      if (tab.value === "bodymap") {
        // Карта/проекция: видимы переключатели Front/Back или Left/Right.
        const projection = page
          .getByRole("button", { name: /Front|Спина|Back|Спереди|Сзади/i })
          .first();
        await expect(projection).toBeVisible();
        // Попробуем переключить проекцию, если есть «Сзади»/«Back».
        const back = page.getByRole("button", { name: /Сзади|Back/i }).first();
    ...
    await expect(
      page.getByRole("button", { name: /Печать \/ PDF \(демо\)/ }).first(),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", { name: /Отправить пациенту \(демо\)/ }).first(),
    ).toBeDisabled();

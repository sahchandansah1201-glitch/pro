#!/usr/bin/env node
// Stage 6 · live browser client journey for the production booking helper.
...
export function parseLiveBotBookingE2EArgs(argv = [], env = process.env) {
  const parsed = {
    baseUrl: env.STAGE4M_LIVE_BOT_BOOKING_BASE_URL || env.STAGE4M_LIVE_ADMIN_BASE_URL || DEFAULT_BASE_URL,
...
export function usage() {
  return [
    "Usage:",
    "  npm run e2e:bot-booking:live -- \\",
    "    --base-url https://pro.skindoktor.ru \\",
    `    --confirm-create-test-clinic ${CREATE_TEST_CLINIC_CONFIRMATION}`,
    "",
    "This test creates a temporary patient account in the self-hosted database,",
    "then signs in and verifies the production booking helper through the visible UI.",
  ].join("\n");
}

export function runLiveBotBookingE2E(argv = process.argv.slice(2), { spawn = spawnSync, cwd = process.cwd() } = {}) {
  const parsed = parseLiveBotBookingE2EArgs(argv);
  if (parsed.help) {
    console.log(usage());
    return 0;
  }
  ...
  const args = ["playwright", "test", "e2e/production-bot-booking-live.pw.ts", "--project=chromium"];
  ...
  const result = spawn(command("npx"), args, {
    ...
    env: {
      ...process.env,
      STAGE4M_LIVE_BOT_BOOKING_BASE_URL: parsed.baseUrl.replace(/\/+$/, ""),
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  process.exit(runLiveBotBookingE2E());
}

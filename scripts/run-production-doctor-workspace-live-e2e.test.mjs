...
import {
  parseLiveDoctorE2EArgs,
  runLiveDoctorE2E,
} from "./run-production-doctor-workspace-live-e2e.mjs";
...
test("live doctor e2e parser requires explicit production mutation confirmation", () => {
  const missing = parseLiveDoctorE2EArgs([
    "--base-url",
  ...
  const confirmed = parseLiveDoctorE2EArgs([
    "--base-url",
    "https://pro.example.test",
    "--credentials-file",
    "/root/credentials.txt",
...
test("live doctor e2e runner refuses to start when credentials file is missing", () => {
  const code = runLiveDoctorE2E([
    "--base-url",
    "https://pro.example.test",
    "--credentials-file",
...
test("live doctor e2e blocks when Stage 4M deployment is still running", () => {
  const blocker = deployStatusBlocksLiveE2E({
    deployStatusFile: "/tmp/status.json",
    exists: () => true,
    readFile: () => JSON.stringify({ status: "running", runId: "run-doctor-001" }),
  });

  assert.match(blocker, /deployment is still running.*run-doctor-001/);
});

#!/usr/bin/env node
// Stage 6T · production release archive retention cycle closure receipt guard.
...
const REQUIRED_FILES = [
  ...
  "scripts/stage6t-production-release-archive-retention-cycle-closure-receipt.test.mjs",
  "scripts/check-stage6t-production-release-archive-retention-cycle-closure-receipt.mjs",
  "scripts/check-stage6t-production-release-archive-retention-cycle-closure-receipt.test.mjs",
...
const REQUIRED_TEXT = {
  "deploy/self-hosted/release-archive-retention-cycle-closure-receipt.stage6t.json": [
    "releaseArchiveRetentionCycleClosureManifest",
  ...
  ".github/workflows/stage6t-production-release-archive-retention-cycle-closure-receipt.yml": [
    "name: stage6t-production-release-archive-retention-cycle-closure-receipt",
    "npm run preflight:stage6t",
    "stage6t-production-release-archive-retention-cycle-closure-receipt.md",
    "GITHUB_STEP_SUMMARY",
  ],
};
  ...
  for (const file of PROTECTED_FILES) {
    ...
    for (const pattern of FORBIDDEN_RUNTIME_PATTERNS) {
      if (pattern.test(content)) {
        errors.push(`${file} contains forbidden Stage 6T runtime or live archive evidence coupling: ${pattern}`);
      }
    }
  }
}
...
function validatePackageScripts(errors, root) {
  ...
  for (const script of [
    ...
    "\"check:stage6t\"",
    "\"preflight:stage6t\"",
    "\"receipt:stage6t:dry-run\"",
    "\"receipt:stage6t:report\"",
  ]) {
    if (!packageJson.includes(script)) errors.push(`package.json missing ${script}`);
  }
  const preflightAll = read(root, "scripts/preflight-all.mjs");
  if (!preflightAll.includes("Stage 6T production release archive retention cycle closure receipt preflight")) {
    errors.push("scripts/preflight-all.mjs must include Stage 6T production release archive retention cycle closure receipt preflight");
  }
}

export function collectStage6TChecks({ root = process.cwd() } = {}) {
  const errors = [];
...
export function main() {
  const result = collectStage6TChecks();
  if (!result.ok) {
    console.error("[stage6t-production-release-archive-retention-cycle-closure-receipt] FAILED");
    for (const error of result.errors) console.error(`- ${error}`);
    return 1;
  }
  console.log(`[stage6t-production-release-archive-retention-cycle-closure-receipt] OK (${result.checkedFiles} files checked)`);
  return 0;
}

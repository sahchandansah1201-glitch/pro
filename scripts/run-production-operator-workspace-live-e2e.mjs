#!/usr/bin/env node
// Stage 5M/5P · live browser client journey for production operator workspace.
...
export function parseLiveOperatorE2EArgs(argv = [], env = process.env) {
  const parsed = {
    baseUrl: env.STAGE4M_LIVE_OPERATOR_BASE_URL || env.STAGE4M_LIVE_ADMIN_BASE_URL || DEFAULT_BASE_URL,
    credentialsFile: env.STAGE4M_OPERATOR_SETUP_CREDENTIALS_FILE || env.STAGE4M_ADMIN_CREDENTIALS_FILE || DEFAULT_CREDENTIALS_FILE,
...
export function usage() {
  return [
    ...
    "",
    "This test creates a temporary clinic and operator through the visible admin UI,",
    "then signs in as the operator and verifies the production intake and booking queues.",
  ].join("\n");
}

export function runLiveOperatorE2E(argv = process.argv.slice(2), { spawn = spawnSync, cwd = process.cwd() } = {}) {
  const parsed = parseLiveOperatorE2EArgs(argv);
  if (parsed.help) {
    console.log(usage());
    return 0;
  }
  ...
  const args = ["playwright", "test", "e2e/production-operator-workspace-live.pw.ts", "--project=chromium"];
  ...
  const result = spawn(command("npx"), args, {
    ...
    env: {
      ...process.env,
      STAGE4M_LIVE_OPERATOR_BASE_URL: parsed.baseUrl.replace(/\/+$/, ""),
      STAGE4M_OPERATOR_SETUP_CREDENTIALS_FILE: parsed.credentialsFile,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  process.exit(runLiveOperatorE2E());
}

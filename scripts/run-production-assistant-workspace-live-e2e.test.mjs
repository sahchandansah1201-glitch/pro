...
import {
  parseLiveAssistantE2EArgs,
  runLiveAssistantE2E,
} from "./run-production-assistant-workspace-live-e2e.mjs";
import { CREATE_TEST_CLINIC_CONFIRMATION } from "./stage4m-admin-management-api-smoke.mjs";

test("live assistant e2e parser requires explicit production mutation confirmation", () => {
  const missing = parseLiveAssistantE2EArgs([
    "--base-url",
  ...
  const confirmed = parseLiveAssistantE2EArgs([
    "--base-url",
    "https://pro.example.test",
    "--credentials-file",
    "/root/credentials.txt",
...
test("live assistant e2e runner refuses to start when credentials file is missing", () => {
  const code = runLiveAssistantE2E([
    "--base-url",
    "https://pro.example.test",
    "--credentials-file",
...
test("live assistant e2e blocks when Stage 4M deployment is still running", () => {
  const blocker = deployStatusBlocksLiveE2E({
    deployStatusFile: "/tmp/status.json",
    exists: () => true,
    readFile: () => JSON.stringify({ status: "running", runId: "run-assistant-001" }),
  });

  assert.match(blocker, /deployment is still running.*run-assistant-001/);
});

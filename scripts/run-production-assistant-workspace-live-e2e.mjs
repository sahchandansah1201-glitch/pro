#!/usr/bin/env node
// Stage 4M/assistant · live browser client journey for production capture workspace.
...
export function parseLiveAssistantE2EArgs(argv = [], env = process.env) {
  const parsed = {
    baseUrl: env.STAGE4M_LIVE_ASSISTANT_BASE_URL || env.STAGE4M_LIVE_ADMIN_BASE_URL || DEFAULT_BASE_URL,
    credentialsFile: env.STAGE4M_ASSISTANT_SETUP_CREDENTIALS_FILE || env.STAGE4M_ADMIN_CREDENTIALS_FILE || DEFAULT_CREDENTIALS_FILE,
...
export function usage() {
  return [
    ...
    "",
    "This test creates a temporary assistant capture fixture, signs in as the assistant,",
    "then verifies the production capture upload path and role boundaries.",
  ].join("\n");
}

export function runLiveAssistantE2E(argv = process.argv.slice(2), { spawn = spawnSync, cwd = process.cwd() } = {}) {
  const parsed = parseLiveAssistantE2EArgs(argv);
  if (parsed.help) {
    console.log(usage());
    return 0;
  }
  ...
  const args = ["playwright", "test", "e2e/production-assistant-workspace-live.pw.ts", "--project=chromium"];
  ...
  const result = spawn(command("npx"), args, {
    ...
    env: {
      ...process.env,
      STAGE4M_LIVE_ASSISTANT_BASE_URL: parsed.baseUrl.replace(/\/+$/, ""),
      STAGE4M_ASSISTANT_SETUP_CREDENTIALS_FILE: parsed.credentialsFile,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  process.exit(runLiveAssistantE2E());
}

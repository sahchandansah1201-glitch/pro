...
// Builds an offline, redacted receipt package for the Stage 6Y release archive
// retention next-cycle register. The command reads repository evidence only, performs no
...
const DEFAULT_MANIFEST = "deploy/self-hosted/release-archive-retention-next-cycle-register-receipt.stage6z.json";
const DEFAULT_SUMMARY_PATH = "test-results/stage6z-production-release-archive-retention-next-cycle-register-receipt.md";
const DEFAULT_JSON_PATH = "test-results/stage6z-production-release-archive-retention-next-cycle-register-receipt.json";
const DEFAULT_NOW = "2026-05-19T12:30:00.000Z";
const REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

const REQUIRED_INPUT_KEYS = [
  "stage6y_release_archive_retention_next_cycle_register",
  "stage6y_release_archive_retention_next_cycle_register_generator",
...
const REQUIRED_SECTION_KEYS = [
  ...
  "external_next_cycle_register_receipt_reference",
  "retention_next_cycle_schedule_receipt_reference",
  "retention_next_cycle_owner_receipt_reference",
  "retention_next_cycle_disposal_hold_receipt_reference",
  "retention_next_cycle_review_receipt_reference",
  "production_boundary",
  "next_cycle_entrypoints",
];

const REQUIRED_RECEIPT_FIELD_KEYS = [
  "archive_retention_next_cycle_receipt_id_reference",
  "archive_retention_next_cycle_register_id_reference",
  "archive_retention_next_cycle_owner_signoff_receipt_reference",
  "retention_next_cycle_review_receipt_reference",
  "retention_next_cycle_disposal_hold_receipt_reference",
];
...
export class Stage6ZReleaseArchiveRetentionNextCycleRegisterReceiptError extends Error {
  constructor(details = []) {
    super("Stage 6Z production release archive retention next-cycle register receipt failed validation.");
    this.name = "Stage6ZReleaseArchiveRetentionNextCycleRegisterReceiptError";
  ...
  if (typeof value === "string") {
    for (const { code, pattern } of LEAK_PATTERNS) {
      if (pattern.test(value)) {
        details.push({ field: path, message: `Archive retention next-cycle register receipt contains forbidden value: ${code}.` });
        return;
      }
    }
    return;
...
function validateBoundary(boundary, details) {
  ...
  const expected = {
    ...
    releaseArchiveRetentionNextCycleRegisterReceiptBundledInRepository: true,
    ...
    externalArchiveRetentionNextCycleRegisterReceiptStoredOutsideGit: true,
    externalArchiveRetentionNextCycleOwnerReceiptStoredOutsideGit: true,
    externalArchiveRetentionNextCycleDecisionReceiptStoredOutsideGit: true,
    ...
    archiveRetentionNextCycleRegisterReceiptOutcomeKnownToRepository: false,
...
function validatePolicy(policy, details) {
  ...
  if (policy.nextStageHypothesis !== "Stage 7A") {
    details.push({ field: "receiptPolicy.nextStageHypothesis", message: "Expected Stage 7A." });
  }
}

export function readReleaseArchiveRetentionNextCycleRegisterReceiptManifest(path = DEFAULT_MANIFEST) {
  return readJsonFile(path);
}

export function validateReleaseArchiveRetentionNextCycleRegisterReceiptManifest(rawManifest) {
  const details = [];
  if (!isPlainObject(rawManifest)) {
    throw new Stage6ZReleaseArchiveRetentionNextCycleRegisterReceiptError([{ field: "manifest", message: "manifest must be an object." }]);
  }
  const manifest = { ...rawManifest };
  if (manifest.stage !== "6Z") details.push({ field: "stage", message: "Expected 6Z." });
  if (manifest.packageId !== "stage6z-production-release-archive-retention-next-cycle-register-receipt") {
    details.push({ field: "packageId", message: "Unexpected packageId." });
  }
  manifest.releaseArchiveRetentionNextCycleRegisterManifest = validateSafeRelativePath(
  ...
  if (details.length > 0) throw new Stage6ZReleaseArchiveRetentionNextCycleRegisterReceiptError(details);
  return manifest;
}

export function detectReleaseArchiveRetentionNextCycleRegisterReceiptLeaks(value) {
  const details = [];
...
export function buildProductionReleaseArchiveRetentionNextCycleRegisterReceipt({
  manifest = readReleaseArchiveRetentionNextCycleRegisterReceiptManifest(),
  root = REPO_ROOT,
  generatedAt = DEFAULT_NOW,
} = {}) {
  const validated = validateReleaseArchiveRetentionNextCycleRegisterReceiptManifest(manifest);
  const retentionRegisterManifest = validateReleaseArchiveRetentionNextCycleRegisterManifest(
    readReleaseArchiveRetentionNextCycleRegisterManifest(validated.releaseArchiveRetentionNextCycleRegisterManifest),
  );
  const retentionRegister = buildProductionReleaseArchiveRetentionNextCycleRegister({
  ...
  const leakFindings = detectReleaseArchiveRetentionNextCycleRegisterReceiptLeaks(validated);
  const readyForExternalReleaseArchiveRetentionNextCycleRegisterReceipt =
  ...
  return {
    ...
    generatedAt,
    status: readyForExternalReleaseArchiveRetentionNextCycleRegisterReceipt ? "ready" : "blocked",
    readyForExternalReleaseArchiveRetentionNextCycleRegisterReceipt,
    releaseArchiveRetentionNextCycleRegister: {
      status: retentionRegister.status,
      readyForExternalReleaseArchiveRetentionNextCycleRegister:
        retentionRegister.readyForExternalReleaseArchiveRetentionNextCycleRegister,
    ...
    releaseArchiveRetentionNextCycleRegisterReceiptStoredInGit: true,
    releaseArchiveRetentionNextCycleRegisterStoredInGit: true,
    ...
    externalArchiveRetentionNextCycleRegisterReceiptStoredOutsideGit:
      validated.productBoundary.externalArchiveRetentionNextCycleRegisterReceiptStoredOutsideGit,
    externalArchiveRetentionNextCycleOwnerReceiptStoredOutsideGit:
      validated.productBoundary.externalArchiveRetentionNextCycleOwnerReceiptStoredOutsideGit,
    externalArchiveRetentionNextCycleDecisionReceiptStoredOutsideGit:
      validated.productBoundary.externalArchiveRetentionNextCycleDecisionReceiptStoredOutsideGit,
    ...
    archiveRetentionNextCycleRegisterReceiptOutcomeKnownToRepository:
      validated.productBoundary.archiveRetentionNextCycleRegisterReceiptOutcomeKnownToRepository,
...
export function renderProductionReleaseArchiveRetentionNextCycleRegisterReceiptMarkdown(report) {
  const lines = [
    "# Stage 6Z production release archive retention next-cycle register receipt",
    "",
    `- Generated at: \`${report.generatedAt}\``,
    `- Status: \`${report.status}\``,
    `- Ready for external release archive retention next-cycle register receipt: \`${report.readyForExternalReleaseArchiveRetentionNextCycleRegisterReceipt}\``,
    `- Stage 6Y archive retention next-cycle register status: \`${report.releaseArchiveRetentionNextCycleRegister.status}\``,
    `- Ready for external archive retention next-cycle register: \`${report.releaseArchiveRetentionNextCycleRegister.readyForExternalReleaseArchiveRetentionNextCycleRegister}\``,
    `- Release archive retention next-cycle register receipt stored in git: \`${report.releaseArchiveRetentionNextCycleRegisterReceiptStoredInGit}\``,
    `- Release archive retention next-cycle register stored in git: \`${report.releaseArchiveRetentionNextCycleRegisterStoredInGit}\``,
    `- Stage 6X final closure reconciliation receipt stored in git: \`${report.releaseArchiveRetentionCycleFinalClosureReconciliationReceiptStoredInGit}\``,
    ...
    `- External archive retention next-cycle register receipt stored outside git: \`${report.externalArchiveRetentionNextCycleRegisterReceiptStoredOutsideGit}\``,
    `- External archive retention next-cycle owner receipt stored outside git: \`${report.externalArchiveRetentionNextCycleOwnerReceiptStoredOutsideGit}\``,
    `- External archive retention next-cycle decision receipt stored outside git: \`${report.externalArchiveRetentionNextCycleDecisionReceiptStoredOutsideGit}\``,
    ...
    `- Archive retention next-cycle register receipt outcome known to repository: \`${report.archiveRetentionNextCycleRegisterReceiptOutcomeKnownToRepository}\``,
  ...
  lines.push("- The repository stores the retention next-cycle register receipt schema, redacted field names, gates, and safe pointers.");
...
export function parseStage6ZArgs(argv = process.argv.slice(2)) {
  const parsed = {
    dryRun: false,
...
export function runStage6ZProductionReleaseArchiveRetentionNextCycleRegisterReceipt(options = {}) {
  const manifest = readReleaseArchiveRetentionNextCycleRegisterReceiptManifest(options.manifest ?? DEFAULT_MANIFEST);
...
export function main(argv = process.argv.slice(2)) {
  try {
    const args = parseStage6ZArgs(argv);
    ...
    console.error(`[stage6z-production-release-archive-retention-next-cycle-register-receipt] failed: ${error.message}`);
    if (error?.details) {

...
import { collectStage6JChecks } from "./check-stage6j-production-release-archive-handoff-receipt.mjs";
...
test("Stage 6J guard passes for the repository tree", () => {
  const result = collectStage6JChecks({ root: ROOT });
  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.equal(result.checkedFiles, 7);
});

test("Stage 6J guard reports missing required files", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage6j-check-missing-"));
  try {
    const result = collectStage6JChecks({ root: dir });
...
test("Stage 6J guard blocks forbidden managed runtime coupling", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage6j-check-forbidden-"));
  try {
    copyRequiredTree(dir);
    const script = join(dir, "scripts/stage6j-production-release-archive-handoff-receipt.mjs");
    writeFileSync(script, `${readFileSync(script, "utf8")}\nconst bad = "SUPABASE_URL";\n`);
    const result = collectStage6JChecks({ root: dir });
    assert.equal(result.ok, false);
    assert.match(result.errors.join("\n"), /forbidden Stage 6J runtime/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

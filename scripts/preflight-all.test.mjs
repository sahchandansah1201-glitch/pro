...
test("preflight all command list covers deterministic local gates", () => {
  ...
  assert.match(commands[87], /npm(\.cmd)? run ci:release-status-sync/);
  assert.match(commands[88], /npm(\.cmd)? run check:preflight-all-gate/);
  assert.match(commands[89], /npm(\.cmd)? run check:project-memory/);
  assert.match(commands[90], /scripts\/check-no-deno-locks\.mjs/);
  assert.equal(commands[91], "git diff --check");
});

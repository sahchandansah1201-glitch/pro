...
test("preflight all command list covers deterministic local gates", () => {
  ...
  assert.match(commands[40], /npm(\.cmd)? run ci:release-status-sync/);
  assert.match(commands[41], /npm(\.cmd)? run check:preflight-all-gate/);
  assert.match(commands[42], /scripts\/check-no-deno-locks\.mjs/);
  assert.equal(commands[43], "git diff --check");
});

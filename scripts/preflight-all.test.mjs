...
test("preflight all command list covers deterministic local gates", () => {
  ...
  assert.match(commands[116], /npm(\.cmd)? run ci:release-status-sync/);
  assert.match(commands[117], /npm(\.cmd)? run check:preflight-all-gate/);
  assert.match(commands[118], /npm(\.cmd)? run check:project-memory/);
  assert.match(commands[119], /scripts\/check-no-deno-locks\.mjs/);
  assert.equal(commands[120], "git diff --check");
});

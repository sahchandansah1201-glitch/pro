...
test("preflight all command list covers deterministic local gates", () => {
  ...
  assert.match(commands[73], /npm(\.cmd)? run ci:release-status-sync/);
  assert.match(commands[74], /npm(\.cmd)? run check:preflight-all-gate/);
  assert.match(commands[75], /npm(\.cmd)? run check:project-memory/);
  assert.match(commands[76], /scripts\/check-no-deno-locks\.mjs/);
  assert.equal(commands[77], "git diff --check");
});

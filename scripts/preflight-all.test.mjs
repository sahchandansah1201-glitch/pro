...
test("preflight all command list covers deterministic local gates", () => {
  ...
  assert.match(commands[16], /npm(\.cmd)? run ci:release-status-sync/);
  assert.match(commands[17], /npm(\.cmd)? run check:preflight-all-gate/);
  assert.match(commands[18], /scripts\/check-no-deno-locks\.mjs/);
  assert.equal(commands[19], "git diff --check");
});

...
const REQUIRED_TEXT = {
  ...
  "src/pages/doctor/VisitWorkspaceLiveActions.tsx": ["Получить закрытие сверки", "Доработать получение закрытия", "Закрытия сверки получены", "sopPolicyGovernanceEvidenceReconciliationClosureReceiptArchiveClosureReceiptHandoffReceiptReconciliationClosureReceiptSummary"],

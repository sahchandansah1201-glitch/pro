...
import { checkStage14A14Z } from "./check-stage14a-14z-sync-confirmation-ledger.mjs";

const ROOT = new URL("..", import.meta.url).pathname;
const REPO_ROOT = ROOT;
const MANIFEST = "deploy/self-hosted/sync-confirmation-ledger.stage14a-14z.json";

test("Stage 14A-14Z guard passes for repository fixture", () => {
  const result = checkStage14A14Z(REPO_ROOT);

  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.equal(result.checkedFiles, 15);
});

test("Stage 14A-14Z guard rejects missing previous confirmation rule", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage14-guard-"));
  try {
    copyFixture(dir);
    const manifestPath = join(dir, MANIFEST);
    const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
    manifest.ledgerRules = manifest.ledgerRules.filter((rule) => rule.id !== "sync_confirmation_not_memory");
    writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

    const result = checkStage14A14Z(dir);
...
test("Stage 14A-14Z guard rejects mismatched previous confirmation", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage14-guard-"));
  try {
    copyFixture(dir);
    const manifestPath = join(dir, MANIFEST);
...
test("Stage 14A-14Z guard rejects forbidden protected-file markers", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage14-guard-"));
  try {
    copyFixture(dir);
    const docsPath = join(dir, "docs/backend/stage-14a-14z-sync-confirmation-ledger.md");

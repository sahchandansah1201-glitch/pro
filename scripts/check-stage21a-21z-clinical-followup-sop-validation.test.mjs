...
import { checkStage21A21Z } from "./check-stage21a-21z-clinical-followup-sop-validation.mjs";

const REPO_ROOT = new URL("..", import.meta.url).pathname;
const MANIFEST = "deploy/self-hosted/clinical-followup-sop-validation.stage21a-21z.json";

test("Stage 21A-21Z guard passes for repository fixture", () => {
  const result = checkStage21A21Z(REPO_ROOT);
  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.equal(result.checkedFiles, 24);
});

test("Stage 21A-21Z guard rejects managed notification dependency", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage21-guard-"));
  try {
    copyFixture(dir);
    ...
    const result = checkStage21A21Z(dir);
...
test("Stage 21A-21Z guard rejects forbidden protected markers", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage21-guard-"));
  try {
    copyFixture(dir);

...
// Builds an offline, redacted receipt package for the Stage 6M release archive
// final closure. The command reads repository evidence only, performs no
...
const DEFAULT_MANIFEST = "deploy/self-hosted/release-archive-final-closure-receipt.stage6n.json";
const DEFAULT_SUMMARY_PATH = "test-results/stage6n-production-release-archive-final-closure-receipt.md";
const DEFAULT_JSON_PATH = "test-results/stage6n-production-release-archive-final-closure-receipt.json";
const DEFAULT_NOW = "2026-05-19T11:30:00.000Z";
const REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

const REQUIRED_INPUT_KEYS = [
  "stage6m_release_archive_final_closure",
  "stage6m_release_archive_final_closure_generator",
...
const REQUIRED_SECTION_KEYS = [
  ...
  "archive_final_closure_reference",
  "external_archive_final_closure_receipt_reference",
  "final_archive_closure_outcome_reference",
...
const REQUIRED_RECEIPT_FIELD_KEYS = [
  "archive_final_closure_receipt_id_reference",
  "archive_final_closure_outcome_reference",
  "archive_owner_signoff_receipt_reference",
  "retention_followup_receipt_reference",
  "final_closure_receipt_owner_reference",
];
...
export class Stage6NReleaseArchiveFinalClosureReceiptError extends Error {
  constructor(details = []) {
    super("Stage 6N production release archive final closure receipt failed validation.");
    this.name = "Stage6NReleaseArchiveFinalClosureReceiptError";
  ...
  if (typeof value === "string") {
    for (const { code, pattern } of LEAK_PATTERNS) {
      if (pattern.test(value)) {
        details.push({ field: path, message: `Archive final closure receipt contains forbidden value: ${code}.` });
        return;
      }
    }
    return;
...
function validatePolicy(policy, details) {
  ...
  if (!cleanString(policy.finalClosureReceiptOwner)) {
    details.push({ field: "receiptPolicy.finalClosureReceiptOwner", message: "finalClosureReceiptOwner is required." });
  }
}

export function readReleaseArchiveFinalClosureReceiptManifest(path = DEFAULT_MANIFEST) {
  return readJsonFile(path);
}

export function validateReleaseArchiveFinalClosureReceiptManifest(rawManifest) {
  const details = [];
  if (!isPlainObject(rawManifest)) {
    throw new Stage6NReleaseArchiveFinalClosureReceiptError([{ field: "manifest", message: "manifest must be an object." }]);
  }
  const manifest = { ...rawManifest };
  if (manifest.stage !== "6N") details.push({ field: "stage", message: "Expected 6N." });
  if (manifest.packageId !== "stage6n-production-release-archive-final-closure-receipt") {
    details.push({ field: "packageId", message: "Unexpected packageId." });
  }
  manifest.releaseArchiveFinalClosureManifest = validateSafeRelativePath(
  ...
  if (details.length > 0) throw new Stage6NReleaseArchiveFinalClosureReceiptError(details);
  return manifest;
}

export function detectReleaseArchiveFinalClosureReceiptLeaks(value) {
  const details = [];
...
export function buildProductionReleaseArchiveFinalClosureReceipt({
  manifest = readReleaseArchiveFinalClosureReceiptManifest(),
  root = REPO_ROOT,
  generatedAt = DEFAULT_NOW,
} = {}) {
  const validated = validateReleaseArchiveFinalClosureReceiptManifest(manifest);
  const closureManifest = validateReleaseArchiveFinalClosureManifest(
    readReleaseArchiveFinalClosureManifest(validated.releaseArchiveFinalClosureManifest),
  );
  const finalClosure = buildProductionReleaseArchiveFinalClosure({
  ...
  const leakFindings = detectReleaseArchiveFinalClosureReceiptLeaks(validated);
  const readyForExternalReleaseArchiveFinalClosureReceipt =
  ...
  return {
    ...
    generatedAt,
    status: readyForExternalReleaseArchiveFinalClosureReceipt ? "ready" : "blocked",
    readyForExternalReleaseArchiveFinalClosureReceipt,
    releaseArchiveFinalClosure: {
      status: finalClosure.status,
      readyForExternalReleaseArchiveFinalClosure:
        finalClosure.readyForExternalReleaseArchiveFinalClosure,
      externalArchiveFinalClosureStoredOutsideGit:
        finalClosure.externalArchiveFinalClosureStoredOutsideGit,
      archiveFinalClosureOutcomeKnownToRepository:
        finalClosure.archiveFinalClosureOutcomeKnownToRepository,
    },
...
export function renderProductionReleaseArchiveFinalClosureReceiptMarkdown(report) {
  const lines = [
    "# Stage 6N production release archive final closure receipt",
    "",
    `- Generated at: \`${report.generatedAt}\``,
    `- Status: \`${report.status}\``,
    `- Ready for external release archive final closure receipt: \`${report.readyForExternalReleaseArchiveFinalClosureReceipt}\``,
    `- Stage 6M archive final closure status: \`${report.releaseArchiveFinalClosure.status}\``,
    `- Ready for external archive final closure: \`${report.releaseArchiveFinalClosure.readyForExternalReleaseArchiveFinalClosure}\``,
  ...
  lines.push("- The repository stores the final closure receipt schema, redacted field names, gates, and safe pointers.");
  lines.push("- The repository does not store the final archive closure receipt outcome, archive receipt values, reconciliation values, raw archive contents, logs, metrics, credentials, backup contents, or patient-identifying content.");
  lines.push("");
  return lines.join("\n");
}

export function parseStage6NArgs(argv = process.argv.slice(2)) {
  const parsed = {
    dryRun: false,
...
export function runStage6NProductionReleaseArchiveFinalClosureReceipt(options = {}) {
  const manifest = readReleaseArchiveFinalClosureReceiptManifest(options.manifest ?? DEFAULT_MANIFEST);
...
export function main(argv = process.argv.slice(2)) {
  try {
    const args = parseStage6NArgs(argv);
    ...
    console.error(`[stage6n-production-release-archive-final-closure-receipt] failed: ${error.message}`);
    if (error?.details) {

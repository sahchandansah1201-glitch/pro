...
const REQUIRED_TEXT = {
  ...
  "NEXT_ACTIONS.md": ["# NEXT_ACTIONS", "Stage 7A-7C", "hypothesis", "Stage 7D"],
...
const REQUIRED_PROJECT_STATE_FIELDS = [
  ...
  /^\s+path:\s*".*\/pro"/m,
  /^\s+branch:\s*"(main|codex\/[A-Za-z0-9._/-]+)"/m,
  /^\s+head_sha:\s*"[a-f0-9]{40}"/m,
  /^\s+working_tree:\s*"(clean|modified)"/m,
  /^\s+command:\s*"npm run preflight:stage6e"/m,
  /^\s+command:\s*"npm run preflight:stage6f"/m,
  /^\s+command:\s*"npm run preflight:stage6g"/m,
  ...
  /^\s+command:\s*"npm run preflight:stage6l"/m,
  /^\s+command:\s*"npm run preflight:stage6m"/m,
  /^\s+command:\s*"npm run preflight:stage6n"/m,
  /^\s+command:\s*"npm run preflight:stage6o"/m,
  /^\s+command:\s*"npm run preflight:stage6p"/m,
  /^\s+command:\s*"npm run preflight:stage6q"/m,
  /^\s+command:\s*"npm run preflight:stage6r"/m,
  /^\s+command:\s*"npm run preflight:stage6s"/m,
  /^\s+command:\s*"npm run preflight:stage6t"/m,
  /^\s+command:\s*"npm run preflight:stage6u"/m,
  /^\s+command:\s*"npm run preflight:stage6v"/m,
  /^\s+command:\s*"npm run preflight:stage6w"/m,
  /^\s+command:\s*"npm run preflight:stage6x"/m,
  /^\s+command:\s*"npm run preflight:stage6y"/m,
  ...
  /^\s+observation_outcome_known_to_repository:\s*false/m,
  ...
  /^\s+archive_reconciliation_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_final_closure_outcome_known_to_repository:\s*false/m,
  /^\s+archive_final_closure_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_register_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_index_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_closure_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_closure_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_final_closure_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_final_closure_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_final_closure_reconciliation_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_cycle_final_closure_reconciliation_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_next_cycle_outcome_known_to_repository:\s*false/m,
  /^\s+archive_retention_next_cycle_register_receipt_outcome_known_to_repository:\s*false/m,
  /^\s+development_workflow_contract_confirmed:\s*true/m,
...
function assertHypothesesAreExplicit(errors, root) {
  ...
  for (const file of ["HANDOFF.md", "NEXT_ACTIONS.md", "RISKS.md"]) {
    ...
    if (!existsSync(join(root, path))) continue;
    const content = read(root, path);
    if (content.includes("Stage 7D") && !/hypothesis|Hypothesis|Hypotheses/.test(content)) {
      errors.push(`${path} mentions Stage 7D without marking it as a hypothesis`);
    }
  }
}

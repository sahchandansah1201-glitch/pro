#!/usr/bin/env node
// Stage 5Q · external intake import contracts guard.

import { existsSync, readFileSync } from "node:fs";
import { join } from "node:path";
import { fileURLToPath } from "node:url";

const REQUIRED_FILES = [
  "backend/self-hosted/db/migrations/0020_stage5q_external_intake_import_contracts.sql",
  "backend/self-hosted/external-intake-import-repository.mjs",
  "backend/self-hosted/external-intake-import-service.mjs",
  "backend/self-hosted/external-intake-import-repository.test.mjs",
  "backend/self-hosted/external-intake-import-service.test.mjs",
  "backend/self-hosted/openapi.stage5q.json",
  "backend/self-hosted/routes.mjs",
  "deploy/self-hosted/nginx.stage4a.conf",
  "src/lib/self-hosted-external-intake-api.ts",
  "src/lib/self-hosted-external-intake-api.test.ts",
  "src/pages/operator/OperatorBookingRequestsPageLive.tsx",
  "src/pages/operator/OperatorBookingRequestsPage.production.test.tsx",
  "docs/backend/stage-5q-external-intake-import-contracts.md",
  "scripts/check-stage5q-external-intake-import-contracts.mjs",
  "scripts/check-stage5q-external-intake-import-contracts.test.mjs",
  ".github/workflows/stage5q-external-intake-import-contracts.yml",
];

const REQUIRED_TEXT = {
  "backend/self-hosted/db/migrations/0020_stage5q_external_intake_import_contracts.sql": [
    "external_booking_import_batches",
    "clinic_available_slots",
    "storedRawPayload",
    "third-party managed runtime",
  ],
  "backend/self-hosted/external-intake-import-repository.mjs": [
    "buildImportExternalIntakeSql",
    "buildExternalIntakeImportBatchesSql",
    "patient_portal_booking_requests",
    "clinic_available_slots",
  ],
  "backend/self-hosted/external-intake-import-service.mjs": [
    "normalizeExternalIntakeImportPayload",
    "external_intake.import",
    "external_intake.import.list",
    "never calls external systems",
  ],
  "backend/self-hosted/routes.mjs": [
    "OPENAPI_5Q",
    "/api/v1/integrations/booking-imports",
    "stage: \"5Q\"",
    "externalBookingImports",
  ],
  "backend/self-hosted/openapi.stage5q.json": [
    "5Q-external-intake-import-contracts",
    "/api/v1/integrations/booking-imports",
    "ImportRequest",
  ],
  "deploy/self-hosted/nginx.stage4a.conf": ["/openapi.stage5q.json"],
  "src/lib/self-hosted-external-intake-api.ts": [
    "listSelfHostedExternalIntakeImports",
    "/api/v1/integrations/booking-imports",
  ],
  "src/pages/operator/OperatorBookingRequestsPageLive.tsx": [
    "Импорт CRM и рекламных источников",
    "listSelfHostedExternalIntakeImports",
    "локальный backend contract",
  ],
  "docs/backend/stage-5q-external-intake-import-contracts.md": [
    "Stage 5Q",
    "npm run preflight:stage5q",
    "Managed runtime/database dependency: none",
  ],
  ".github/workflows/stage5q-external-intake-import-contracts.yml": [
    "name: stage5q-external-intake-import-contracts",
    "npm run preflight:stage5q",
    "GITHUB_STEP_SUMMARY",
  ],
};

const PROTECTED_RUNTIME_FILES = [
  "backend/self-hosted/external-intake-import-repository.mjs",
  "backend/self-hosted/external-intake-import-service.mjs",
  "backend/self-hosted/openapi.stage5q.json",
  "src/lib/self-hosted-external-intake-api.ts",
  "src/pages/operator/OperatorBookingRequestsPageLive.tsx",
];

const FORBIDDEN_RUNTIME_PATTERNS = [
  /\bapi-read\b/i,
  /\bapi-write\b/i,
  /\bedge function\b/i,
  /\bSUPABASE_/,
  /\bnavigator\.usb\b/i,
  /\bnavigator\.bluetooth\b/i,
  /\bnavigator\.serial\b/i,
  /\bstorage_object_path\b/i,
  /\bsigned_url\b/i,
  /\bmock-data\b/i,
  /\bphysician_text\b/i,
  /\bphysicianText\b/i,
  /\bdoctorVersionText\b/i,
  /fetch\s*\(\s*["'`]https?:\/\//i,
];

function read(root, file) {
  return readFileSync(join(root, file), "utf8");
}

function requireText(errors, root, file, expected) {
  const content = read(root, file);
  for (const text of expected) {
    if (!content.includes(text)) errors.push(`${file} missing required text: ${text}`);
  }
}

function scanRuntimeCoupling(errors, root) {
  for (const file of PROTECTED_RUNTIME_FILES) {
    if (!existsSync(join(root, file))) continue;
    const content = read(root, file);
    for (const pattern of FORBIDDEN_RUNTIME_PATTERNS) {
      if (pattern.test(content)) {
        errors.push(`${file} contains forbidden production runtime coupling: ${pattern}`);
      }
    }
  }
}

function validatePackageScripts(errors, root) {
  const packageJson = read(root, "package.json");
  for (const script of ['"test:stage5q"', '"check:stage5q"', '"preflight:stage5q"']) {
    if (!packageJson.includes(script)) errors.push(`package.json missing ${script}`);
  }
  const preflightAll = read(root, "scripts/preflight-all.mjs");
  if (!preflightAll.includes("Stage 5Q external intake import contracts preflight")) {
    errors.push("scripts/preflight-all.mjs must include Stage 5Q external intake import contracts preflight");
  }
}

export function collectStage5QChecks({ root = process.cwd() } = {}) {
  const errors = [];
  for (const file of REQUIRED_FILES) {
    if (!existsSync(join(root, file))) errors.push(`Missing required file: ${file}`);
  }
  for (const [file, expected] of Object.entries(REQUIRED_TEXT)) {
    if (existsSync(join(root, file))) requireText(errors, root, file, expected);
    else errors.push(`Missing required file (text check): ${file}`);
  }
  scanRuntimeCoupling(errors, root);
  validatePackageScripts(errors, root);
  return { ok: errors.length === 0, errors, checkedFiles: REQUIRED_FILES.length };
}

export function main() {
  const result = collectStage5QChecks();
  if (!result.ok) {
    console.error("[stage5q-external-intake-import-contracts] FAILED");
    for (const error of result.errors) console.error(`- ${error}`);
    return 1;
  }
  console.log(`[stage5q-external-intake-import-contracts] OK (${result.checkedFiles} files checked)`);
  return 0;
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  process.exit(main());
}

...
import { checkStage17A17Z } from "./check-stage17a-17z-clinical-followup-communication.mjs";

const REPO_ROOT = new URL("..", import.meta.url).pathname;
const MANIFEST = "deploy/self-hosted/clinical-followup-communication.stage17a-17z.json";

test("Stage 17A-17Z guard passes for repository fixture", () => {
  const result = checkStage17A17Z(REPO_ROOT);
  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.equal(result.checkedFiles, 26);
});

test("Stage 17A-17Z guard rejects managed notification dependency", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage17-guard-"));
  try {
    copyFixture(dir);
    ...
    const result = checkStage17A17Z(dir);
...
test("Stage 17A-17Z guard rejects forbidden protected markers", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage17-guard-"));
  try {
    copyFixture(dir);

...
test("Stage 4M update plan backs up, pulls, installs, builds and restarts", () => {
  ...
  assert.match(out, /Verify admin clinic create\/edit, account safety, password reset and audit isolation database journey/);
  ...
  assert.match(out, /Verify admin integrations and bot database journey/);
  ...
  assert.match(out, /Verify operator dialog database journey/);
  assert.match(out, /stage4m-operator-dialog-db-smoke\.mjs verify/);

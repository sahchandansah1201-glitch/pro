...
stage_evidence:
  ...
  workflows_present:
    ...
    - ".github/workflows/stage6w-production-release-archive-retention-cycle-final-closure-reconciliation.yml"
hypotheses:
    - "Next logical stage after Stage 6W is Stage 6X."
sources:
  commands:
    - "git status -sb"
  files:
...
test("project memory guard requires Stage 6X uncertainty to be marked as hypothesis", () => {
  const root = makeRoot();

...
stage_evidence:
  ...
  workflows_present:
    ...
    - ".github/workflows/stage6l-production-release-archive-reconciliation-receipt.yml"
hypotheses:
    - "Next logical stage after Stage 6L is Stage 6M."
sources:
  commands:
    - "git status -sb"
  files:
...
test("project memory guard requires Stage 6M uncertainty to be marked as hypothesis", () => {
  const root = makeRoot();

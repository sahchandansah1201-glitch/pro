...
import { collectStage6NChecks } from "./check-stage6n-production-release-archive-final-closure-receipt.mjs";
...
test("Stage 6N guard passes for the repository tree", () => {
  const result = collectStage6NChecks({ root: ROOT });
  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.equal(result.checkedFiles, 7);
});

test("Stage 6N guard reports missing required files", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage6n-check-missing-"));
  try {
    const result = collectStage6NChecks({ root: dir });
...
test("Stage 6N guard blocks forbidden managed runtime coupling", () => {
  const dir = mkdtempSync(join(tmpdir(), "stage6n-check-forbidden-"));
  try {
    copyRequiredTree(dir);
    const script = join(dir, "scripts/stage6n-production-release-archive-final-closure-receipt.mjs");
    writeFileSync(script, `${readFileSync(script, "utf8")}\nconst bad = "SUPABASE_URL";\n`);
    const result = collectStage6NChecks({ root: dir });
    assert.equal(result.ok, false);
    assert.match(result.errors.join("\n"), /forbidden Stage 6N runtime/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

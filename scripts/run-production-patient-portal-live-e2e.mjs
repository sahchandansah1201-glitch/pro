#!/usr/bin/env node
// Stage 5N/5O · live browser client journey for production patient portal.
...
export function parseLivePatientPortalE2EArgs(argv = [], env = process.env) {
  const parsed = {
    baseUrl: env.STAGE4M_LIVE_PATIENT_BASE_URL || env.STAGE4M_LIVE_ADMIN_BASE_URL || DEFAULT_BASE_URL,
    credentialsFile: env.STAGE4M_PATIENT_SETUP_CREDENTIALS_FILE || env.STAGE4M_ADMIN_CREDENTIALS_FILE || DEFAULT_CREDENTIALS_FILE,
...
export function usage() {
  return [
    ...
    "",
    "This test creates a temporary patient portal account in the self-hosted database,",
    "then signs in as the patient and verifies the production portal and booking journey.",
  ].join("\n");
}

export function runLivePatientPortalE2E(argv = process.argv.slice(2), { spawn = spawnSync, cwd = process.cwd() } = {}) {
  const parsed = parseLivePatientPortalE2EArgs(argv);
  if (parsed.help) {
    console.log(usage());
    return 0;
  }
  ...
  const args = ["playwright", "test", "e2e/production-patient-portal-live.pw.ts", "--project=chromium"];
  ...
  const result = spawn(command("npx"), args, {
    ...
    env: {
      ...process.env,
      STAGE4M_LIVE_PATIENT_BASE_URL: parsed.baseUrl.replace(/\/+$/, ""),
      STAGE4M_PATIENT_SETUP_CREDENTIALS_FILE: parsed.credentialsFile,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  process.exit(runLivePatientPortalE2E());
}
